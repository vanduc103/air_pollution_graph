...
class HybridSupervisor(object):
    ...
    def __init__(self, adj_mx, **kwargs):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('Hybrid', reuse=False):
                self._train_model = HybridModel(is_training=True, scaler=scaler,
                                               batch_size=self._data_kwargs['batch_size'],
                                               adj_mx=adj_mx, **self._model_kwargs)

        with tf.name_scope('Validate'):
            with tf.variable_scope('Hybrid', reuse=True):
                self._val_model = HybridModel(is_training=False, scaler=scaler,
                                              batch_size=self._data_kwargs['val_batch_size'],
                                              adj_mx=adj_mx, **self._model_kwargs)

        with tf.name_scope('Test'):
            with tf.variable_scope('Hybrid', reuse=True):
                self._test_model = HybridModel(is_training=False, scaler=scaler,
    ...
    def _get_log_dir(kwargs):
        ...
        if log_dir is None:
            ...
            if filter_type == 'random_walk':
                ...
                filter_type_abbr = 'I'
            run_id = 'hybrid_%s_%d_h_%d_%s_lr_%g_bs_%d_%s/' % (
